package Game.Rally.Ball.BallCollisions 
{
	...
	public class BallCollisions
	{
		...
		private var prevX: Number, prevY: Number, prevT: int, prevR, prevV: Array; 
		...
		public function collise(t: int):void
		{
			...
			if (ball.r[2] <= 0)
			{
				ball.setControlPoint([ball.r[0], ball.r[1], prevR[2]], [prevV[0], prevV[1], -prevV[2]], ball.w, t);
				//bounceOfGround();
			}
		...
		private function collisionWithSquare(square: Array, t:int): Object
		{
			var k1 = MathLib.pointSquareDistance(prevR, square);
			var k2 = MathLib.pointSquareDistance(ball.r, square);
			if (k1 * k2 <= 0 && k1 != 0)
			{							
				var k=k1/(k2-k1);
				return { exists: true, r: [prevR[0]+(ball.r[0]-prevR[0])*k, prevR[1]+(ball.r[1]-prevR[1])*k, prevR[2]+(ball.r[2]-prevR[2])*k], t: prevT+(t-prevT)*k};
			}
			else
			{
				return { exists: false };
